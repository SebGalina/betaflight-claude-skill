...
7. For **closed-loop frequency response (Bode)** plus a full guided tuning report, when the log was recorded with Betaflight's chirp generator (`debug_mode = CHIRP`), run `python -m scripts.chirp_analysis <log.bbl> --html report.html` (pass several logs for a before/after overlay; add `--lang en` for English). The firmware adds a swept sine onto `currentPidSetpoint` cycling roll → pitch → yaw. **The firmware debug mapping is auto-detected**: current BF (2025.12+) logs only `debug[0] = 5000·sinarg`, so the tool reconstructs the excitation `sin(debug[0]/5000)`, the instantaneous sweep frequency from the unwrapped phase, and segments axes by setpoint energy; the legacy `debug[1..3]` (axis / freq / raw excitation) path is kept as a fallback. The FRF `H(f) = Pxy/Pxx` (gain dB / phase deg / coherence, diagnosis gated > 0.8) defaults to the calibrated `setpoint[i]` input. The self-contained HTML (vanilla-JS canvas, no external deps) is a **bilingual (FR/EN, live toggle) guided assistant** ordered Filtering → PID → History: per-axis Bode + **step response**; a **gyro noise PSD** (floor-referenced raw vs filtered, with eRPM-derived **motor-harmonic bands**, filter cut-off lines, and disable-candidate analysis); a **chirp spectrogram** (log frequency); a throttle × frequency resonance map; a **multi-pass overlay** with an exhaustive settings-comparison table; and **data-driven** observations (deterministic Python rules, no LLM) read from the PID/filter settings in the log header. Phase margin is reported with an uncertainty — prefer the overlaid curves / step response for before-after. `--json` is machine-readable; `--history`/`--no-history` control the accumulated `chirp_history.json`. Confirm chirp support first with `dump | grep chirp` (the `USE_CHIRP` build feature). See `references/chirp-tuning.md`. **Never enable CHIRP on the ground** (bug betaflight/betaflight#15012).
