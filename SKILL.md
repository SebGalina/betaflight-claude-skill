...
7. For **closed-loop frequency response (Bode)**, when the log was recorded with Betaflight's chirp generator (`debug_mode = CHIRP`), run `python -m scripts.chirp_analysis <log.bbl> --html report.html`. The firmware adds a swept sine onto `currentPidSetpoint` (cycling roll → pitch → yaw) and logs the excitation in `debug[]` (`debug[1]` = active axis, `debug[3]` = chirp reference). The script computes the FRF `H(f) = Pxy/Pxx` per axis → **gain (dB)**, **phase (deg)**, and **coherence**, gates the diagnosis to where coherence > 0.8, and renders a self-contained HTML report (Bode + a throttle × frequency resonance heatmap, no external dependencies). Reads gain/phase shape directly: 40–60 Hz bump → P/D overshoot; narrow peak → frame resonance (dynamic notch); HF roll-off → low-pass. Confirm chirp support first with `dump | grep chirp` (the `USE_CHIRP` build feature). See `references/chirp-tuning.md`. **Never enable CHIRP on the ground** (bug betaflight/betaflight#15012).
...
- `scripts/chirp_analysis.py` — Closed-loop chirp frequency-response analyser (`debug_mode = CHIRP`): per-axis Bode (gain dB / phase deg / coherence) via Welch cross-spectral FRF, diagnosis gated on coherence > 0.8, plus a throttle × frequency resonance map; `--html` writes a self-contained report (vanilla-JS canvas, no external dependencies), `--json` machine-readable (see `references/chirp-tuning.md`)
