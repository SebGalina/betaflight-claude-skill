...
- `scripts/blackbox_signal.py` — Shared signal-analysis helpers: in-process bbl→DataFrame decode (via `blackbox_decoder`, with an isolated `analyze_blackbox --csv` subprocess fallback), sample-rate estimate, flight-activity mask; used by `spectral_analysis.py`, `step_response.py` and `chirp_analysis.py`
...
- `scripts/spectral_analysis.py` — Noise spectrum / FFT analyser (gyro or D-term): Welch PSD per axis, automatic peak extraction, harmonic-series grouping (motor noise vs frame resonance vs broadband), source diagnosis; `--plot` renders PSD + spectrogram
